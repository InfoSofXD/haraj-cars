...
class _RoleBasedDashboardState extends State<RoleBasedDashboard> {
  ...
  UserRole get userRole => currentUser?.role ?? UserRole.client;
  UserPermissions get permissions => currentUser?.permissions ?? UserPermissions.forRole(UserRole.client);
  ...
  Future<void> _loadStatistics() async {
    try {
      ...
      // Get total users count (only for super admins)
      int totalUsers = 0;
      if (permissions.canManageUsers) {
        try {
      ...
      // Get total admins count (only for super admins)
      int totalAdmins = 0;
      ...
      children: [
        ...
        Row(
          children: [
            ...
            if (permissions.canAddCars && permissions.canViewCars)
              const SizedBox(width: 16),
